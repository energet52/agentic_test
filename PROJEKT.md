...
Postupuj shora dolů. U každé fáze je **co udělat**, **co se naučíš** a
**kterého agenta/skill použít**. Odškrtávej si splněné body.
...
- [ ] **Vyzkoušej, že umíš spustit víc agentů paralelně** (jako `/tym-revize`
      u Hada) — třeba `tester` + `code-reviewer` + `kontrolor-pristupnosti` naráz.
